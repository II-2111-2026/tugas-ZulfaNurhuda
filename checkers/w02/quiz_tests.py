"""
[AUTOGRADING TEST — w02]

Script testcase yang disediakan untuk assignment minggu ke-02.
Jawaban mahasiswa diambil dari "submissions/w02/answers.py"
"""
...
from submissions.w02 import answers as A
...
def test_q08():
    assert math.isclose(_num(A.q08()), 0.9, rel_tol=0.0, abs_tol=0.0005)

def test_q09():
    assert math.isclose(_num(A.q09()), 0.95, rel_tol=0.0, abs_tol=0.0005)

def test_q10():
    assert _num(A.q10()) == 0.0

def test_q11():
    assert math.isclose(_num(A.q11()), 0.5, rel_tol=0.0, abs_tol=0.0005)

def test_q12():
    assert math.isclose(_num(A.q12()), 0.3, rel_tol=0.0, abs_tol=0.0005)
