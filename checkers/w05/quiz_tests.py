"""
[AUTOGRADING TEST — w05]

Script testcase yang disediakan untuk assignment minggu ke-05.
Jawaban mahasiswa diambil dari "submissions/w05/answers.py"
"""
...
from submissions.w05 import answers as A
...
def test_q08():
    assert math.isclose(_num(A.q08()), 0.6, rel_tol=0.0, abs_tol=0.0005)

def test_q09():
    assert math.isclose(_num(A.q09()), 0.24, rel_tol=0.0, abs_tol=0.0005)

def test_q10():
    assert _num(A.q10()) == 16.0

def test_q11():
    assert _num(A.q11()) == 2.0

def test_q12():
    assert _num(A.q12()) == 36.0
