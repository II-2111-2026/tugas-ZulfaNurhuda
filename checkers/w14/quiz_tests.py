"""
[AUTOGRADING TEST — w14]

Script testcase yang disediakan untuk assignment minggu ke-14.
Jawaban mahasiswa diambil dari "submissions/w14/answers.py"
"""
...
from submissions.w14 import answers as A
...
def test_q08():
    assert math.isclose(_num(A.q08()), 0.8, rel_tol=0.0, abs_tol=0.0005)

def test_q09():
    assert _num(A.q09()) == 950.0

def test_q10():
    assert math.isclose(_num(A.q10()), 0.8, rel_tol=0.0, abs_tol=0.0005)

def test_q11():
    assert _num(A.q11()) == 2.0

def test_q12():
    assert _num(A.q12()) == 1.0
