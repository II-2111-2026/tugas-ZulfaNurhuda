---
title: "Laporan Minggu 02"
subtitle: "Ringkasan Eksekusi & Inspeksi Kode"
---

::: {.callout-tip title="Tujuan Laporan Ini"}
Laporan ini di-*generate* secara otomatis untuk menampilkan hasil eksekusi kodemu dari `submissions/w01/answers.py` dalam format yang **rapi, terstruktur, dan nyaman dibaca**.

✅ **Sumber Kebenaran Tunggal:** Sistem *autograder* tetap akan menilai langsung dari file `.py` milikmu, laporan ini murni hanya sebagai *dashboard* visualisasi.
:::
...
```bash
pytest -q checkers/w02/quiz_tests.py
```
...
```bash
quarto render submissions/w02/laporan.qmd
```
:::
...
prob_tasks = {"q08", "q09", "q10", "q11", "q12"}
count_tasks = set()
var_tasks = set()
corr_tasks = set()
bool_num_tasks = set()
...
for i in range(1, 13):
    ...
    if status == "OK":
        if name in prob_tasks and not _in_0_1(val):
            notes.append("⚠️ Bukan probabilitas (0..1)")
        if name in count_tasks and int(val) != val:
            notes.append("⚠️ Bukan bilangan bulat")
        if name in var_tasks and _is_number(val) and float(val) < 0:
            notes.append("⚠️ Nilai tidak boleh negatif")
        if name in corr_tasks and _is_number(val) and not (-1.0 <= float(val) <= 1.0):
            notes.append("⚠️ Korelasi harus rentang -1.0 .. 1.0")
        if name in bool_num_tasks and val not in (0, 1):
            notes.append("⚠️ Jawaban harus angka 0 atau 1")

        if isinstance(val, float) and (math.isnan(val) or math.isinf(val)):
            notes.append("⚠️ Terdapat NaN/Inf")

    if status in ("ERR", "MISSING"):
        notes.append("❌ Perlu diperbaiki")
    elif status == "TODO":
        notes.append("📝 Menunggu implementasi")
    elif not notes:
        notes.append("✅ Aman")
...
for name, status, val, err in details:
    ...
    if status == "OK":
        ...
        print(f"> ⏳ *Fungsi ini belum diimplementasikan.* \n\n")
    else:
